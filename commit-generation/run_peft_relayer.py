...
from peft import LoraConfig, TaskType, get_peft_model, PrefixTuningConfig, PromptTuningInit, PromptTuningConfig
...
def train(args, train_examples, train_dataset, eval_examples, eval_dataset, mymodel, tokenizer):
    ...
    # multi-gpu training
    if args.n_gpu > 1:
        mymodel = torch.nn.DataParallel(mymodel, device_ids=args.gpu_list)
...
def main(args):
    ...
    if args.method in ["prefix"]:
        ...
        peft_config = PromptTuningConfig(
            ...
            tokenizer_name_or_path=args.actual_name)
    ...
    if layer_pos >= 0:
        while layer_pos > 0:      # <= 11
            locked_layer = "block." + str(layer_pos - 1) + "."    # str(layer_pos)

            for name, param in mymodel.named_parameters():
                # if param.requires_grad == True:
                if locked_layer in name:
