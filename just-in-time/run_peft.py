...
from peft import LoraConfig, TaskType, get_peft_model, PromptTuningConfig, PrefixTuningConfig, PromptTuningInit
...
def train(args, train_dataset, eval_dataset, mymodel):
    ...
    for idx in range(args.epochs):
        ...
        for step, batch in enumerate(bar):
            ...
            mymodel.train()
            loss, prob = mymodel(input_ids, input_mask, manual_features, label)
            if args.n_gpu > 1:
                loss = loss.mean()
...
def main(args):
    ...
    if args.method in ["prefix"]:
        ...
        if args.pretrained_model in ["codet5"]:
            ...
            assert False, "Invalid Pretrained Model!"
